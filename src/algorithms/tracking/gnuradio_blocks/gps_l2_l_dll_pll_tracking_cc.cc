 ...
 */
#define GPS_L2L_CN0_ESTIMATION_SAMPLES 10
#define GPS_L2L_MINIMUM_VALID_CN0 25
#define GPS_L2L_MAXIMUM_LOCK_FAIL_COUNTER 50
#define GPS_L2L_CARRIER_LOCK_THRESHOLD 0.75


using google::LogMessage;

gps_l2_l_dll_pll_tracking_cc_sptr
gps_l2_l_dll_pll_make_tracking_cc(
        long if_freq,
        long fs_in,
...
{
    return gps_l2_l_dll_pll_tracking_cc_sptr(new gps_l2_l_dll_pll_tracking_cc(if_freq,
            fs_in, vector_length, dump, dump_filename, pll_bw_hz, dll_bw_hz, early_late_space_chips));
}


void gps_l2_l_dll_pll_tracking_cc::forecast (int noutput_items,
...
gps_l2_l_dll_pll_tracking_cc::gps_l2_l_dll_pll_tracking_cc(
        long if_freq,
        long fs_in,
        unsigned int vector_length,
        bool dump,
        ...
        float dll_bw_hz,
        float early_late_space_chips) :
        gr::block("gps_l2_l_dll_pll_tracking_cc", gr::io_signature::make(1, 1, sizeof(gr_complex)),
                gr::io_signature::make(1, 1, sizeof(Gnss_Synchro)))
{
    ...
    d_carrier_loop_filter=Tracking_2nd_PLL_filter(GPS_L2_L_PERIOD);
    d_code_loop_filter=Tracking_2nd_DLL_filter(GPS_L2_L_PERIOD);
    ...
    d_ca_code = static_cast<gr_complex*>(volk_gnsssdr_malloc(static_cast<int>(GPS_L2_L_CODE_LENGTH_CHIPS) * sizeof(gr_complex), volk_gnsssdr_get_alignment()));
    ...
    d_code_freq_chips = GPS_L2_L_CODE_RATE_HZ;
    ...
    d_Prompt_buffer = new gr_complex[GPS_L2L_CN0_ESTIMATION_SAMPLES];
    d_carrier_lock_test = 1;
    d_CN0_SNV_dB_Hz = 0;
    d_carrier_lock_fail_counter = 0;
    d_carrier_lock_threshold = GPS_L2L_CARRIER_LOCK_THRESHOLD;
...
void gps_l2_l_dll_pll_tracking_cc::start_tracking()
{
    /*
    ...
    d_code_freq_chips = radial_velocity * GPS_L2_L_CODE_RATE_HZ;
    d_code_phase_step_chips = static_cast<double>(d_code_freq_chips) / static_cast<double>(d_fs_in);
    T_chip_mod_seconds = 1/d_code_freq_chips;
    T_prn_mod_seconds = T_chip_mod_seconds * GPS_L2_L_CODE_LENGTH_CHIPS;
    T_prn_mod_samples = T_prn_mod_seconds * static_cast<float>(d_fs_in);

    d_current_prn_length_samples = round(T_prn_mod_samples);

    double T_prn_true_seconds = GPS_L2_L_CODE_LENGTH_CHIPS / GPS_L2_L_CODE_RATE_HZ;
    ...
    gps_l2c_l_code_gen_complex(d_ca_code, d_acquisition_gnss_synchro->PRN);

    multicorrelator_cpu.set_local_code_and_taps(static_cast<int>(GPS_L2_L_CODE_LENGTH_CHIPS), d_ca_code, d_local_code_shift_chips);
    for (int n = 0; n < d_n_correlator_taps; n++)
        {
            d_correlator_outs[n] = gr_complex(0,0);
        }
...
int gps_l2_l_dll_pll_tracking_cc::general_work (int noutput_items __attribute__((unused)), gr_vector_int &ninput_items __attribute__((unused)),
    ...
    if (d_enable_tracking == true)
        {
            ...
            d_code_freq_chips = GPS_L2_L_CODE_RATE_HZ + ((d_carrier_doppler_hz * GPS_L2_L_CODE_RATE_HZ) / GPS_L2_FREQ_HZ);
            ...
            double code_error_filt_secs = (GPS_L2_L_PERIOD * code_error_filt_chips) / GPS_L2_L_CODE_RATE_HZ; //[seconds]

            // ################## CARRIER AND CODE NCO BUFFER ALIGNEMENT #######################
            // keep alignment parameters for the next input buffer
            // Compute the next buffer length based in the new period of the PRN sequence and the code phase error estimation
            double T_chip_seconds = 1.0 / d_code_freq_chips;
            double T_prn_seconds = T_chip_seconds * GPS_L2_L_CODE_LENGTH_CHIPS;
